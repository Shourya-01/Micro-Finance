            ...
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {cardsData.map((card, index) => (
                <SingleCard
                  ...
                  CardDescription={card.LoanType}
                  Carddet1={card.LoanAmount}
                  CardDet2={card.InterestRate}
                  CardDet3={card.ContactNumber}
                  Cardemail={card.LemailId}
                  Button="Apply"
...
const SingleCard = ({
  ...
  const handleSubmit = (Lemail) => async (e) => {
    e.preventDefault();
    console.log("hey there 2");
    const formwithEmail = { ...form, Lemail };
    const response = await fetch("http://localhost:8080/demo2", {
      method: "POST",
  ...
  return (
    <div className="mb-4 overflow-hidden rounded-lg bg-gray-200 border border-black shadow-md duration-300 hover:shadow-3 dark:bg-dark-2 dark:shadow-card dark:hover:shadow-3">
      <img src={image} alt="" className="w-full" />
      <div className="p-3 text-center sm:p-4 md:p-2 xl:p-4">
        <h3>
          <a
            href="#"
            className="mb-2 block text-sm font-semibold text-dark hover:text-primary dark:text-black sm:text-base md:text-sm lg:text-base xl:text-base 2xl:text-sm"
          >
        ...
        >
          <p
            ...
            className="mb-2 text-xs leading-relaxed text-body-color dark:text-dark-6"
          >
        ...
        >
          <p
            ...
            className="mb-2 text-xs leading-relaxed text-body-color dark:text-dark-6"
          >
            <span className="font-semibold">Interest Rate :</span> {CardDet2}%
          </p>
          <p className="mb-2 text-xs leading-relaxed text-body-color dark:text-dark-6">
            <span className="font-semibold">Contact Number :</span> {CardDet3}
          </p>
        </div>

        {Button && (
          <button
            onClick={openPopup} // Attach onClick event to the button
      ...
      {isOpen && (
        <div
          ...
          className="fixed top-0 left-0 w-full bg-gray-900 bg-opacity-50 flex items-center justify-center h-screen"
        >
          <div className="bg-white p-8 rounded shadow-lg absolute top-10 left-1/2 transform -translate-x-1/2">
            <h2
            ...
            <form
              ...
              onSubmit={handleSubmit({ Cardemail })}
              className="space-y-4"
            >
